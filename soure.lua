-- Vortex Hub v2.66 - Complete Working Version
local VERSION = "2.66"
...
-- CONFIG
local cfg = {
    aimbot = {enabled = false, teamCheck = true, fov = 75, smoothness = 0.25},
    softAim = {enabled = false, strength = 3},
    silentAim = {enabled = false},
    esp = {enabled = true, boxes = true, names = true, health = true, tracers = false, bones = false, distance = true, rainbow = false, maxDistance = 3000, maxPlayers = 30},
...
local function updateAimbot()
    if cfg.aimbot.enabled and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
        ...
        if target then
            local smooth = cfg.aimbot.smoothness * (0.7 + math.random() * 0.6)
...
local function startFly()
    ...
    flyPart = Instance.new("Part"); flyPart.Name = "Fly"; flyPart.Size = Vector3.new(1,0.2,1); flyPart.Transparency = 1; flyPart.CanCollide = false; flyPart.Anchored = false; flyPart.CFrame = root.CFrame; flyPart.Parent = workspace
    flyVel = Instance.new("BodyVelocity", flyPart); flyVel.Velocity = Vector3.zero; flyVel.MaxForce = Vector3.new(1e5,1e5,1e5); flyVel.P = 1e4
    flyWeld = Instance.new("WeldConstraint", flyPart); flyWeld.Part0 = flyPart; flyWeld.Part1 = root
...
espGui.Name = "ESP"; espGui.ResetOnSpawn = false; espGui.Parent = player.PlayerGui
local espObjs = {}
local function createESP()
    local o = {box = {}, bones = {}}
    for _, n in ipairs({"t","b","l","r"}) do
        local f = Instance.new("Frame", espGui); f.BackgroundColor3 = Color3.new(1,1,1); f.BackgroundTransparency = 0; f.BorderSizePixel = 0; f.Size = UDim2.new(0,0,0,0); f.Visible = false; o.box[n] = f
    end
    local hb = Instance.new("Frame", espGui); hb.BackgroundColor3 = Color3.fromRGB(20,20,20); hb.BorderSizePixel = 0; hb.Visible = false; o.healthBG = hb
    local hf = Instance.new("Frame", espGui); hf.BackgroundColor3 = Color3.fromRGB(34,197,94); hf.BorderSizePixel = 0; hf.Visible = false; o.healthBar = hf
    local nl = Instance.new("TextLabel", espGui); nl.BackgroundTransparency = 1; nl.TextColor3 = Color3.new(1,1,1); nl.TextStrokeTransparency = 0; nl.TextStrokeColor3 = Color3.new(0,0,0); nl.TextSize = 12; nl.Font = Enum.Font.GothamBold; nl.Size = UDim2.new(0,300,0,16); nl.TextXAlignment = Enum.TextXAlignment.Center; nl.Visible = false; o.nameLabel = nl
    local tr = Instance.new("Frame", espGui); tr.AnchorPoint = Vector2.new(0.5,0.5); tr.BackgroundColor3 = Color3.new(1,1,1); tr.BackgroundTransparency = 0; tr.BorderSizePixel = 0; tr.Size = UDim2.new(0,0,0,1); tr.Visible = false; o.tracer = tr
    for i = 1, 15 do
        local b = Instance.new("Frame", espGui); b.AnchorPoint = Vector2.new(0.5,0.5); b.BackgroundColor3 = Color3.new(1,1,1); b.BackgroundTransparency = 0; b.BorderSizePixel = 0; b.Size = UDim2.new(0,0,0,1); b.Visible = false; o.bones[i] = b
    end
    return o
end
local bonePairs = {{"Head","UpperTorso"},{"UpperTorso","LowerTorso"},{"LowerTorso","HumanoidRootPart"},{"UpperTorso","RightUpperArm"},{"RightUpperArm","RightLowerArm"},{"RightLowerArm","RightHand"},{"UpperTorso","LeftUpperArm"},{"LeftUpperArm","LeftLowerArm"},{"LeftLowerArm","LeftHand"},{"LowerTorso","RightUpperLeg"},{"RightUpperLeg","RightLowerLeg"},{"RightLowerLeg","RightFoot"},{"LowerTorso","LeftUpperLeg"},{"LeftUpperLeg","LeftLowerLeg"},{"LeftLowerLeg","LeftFoot"}}
...
RunService.RenderStepped:Connect(function()
    if not cfg.esp.enabled then
        for _, o in pairs(espObjs) do
            ...
            o.healthBG.Visible = false; o.healthBar.Visible = false; o.nameLabel.Visible = false; o.tracer.Visible = false
    ...
    for plr, o in pairs(espObjs) do
        ...
        count = count + 1
        local color = cfg.esp.rainbow and Color3.fromHSV((tick()*0.05)%1,1,1) or (plr.Team and plr.Team.TeamColor.Color) or Color3.fromRGB(255,255,255)
...
-- ====== MOVEMENT SYSTEMS ======
RunService.RenderStepped:Connect(function()
    if cfg.combat.spinBot then local root = character:FindFirstChild("HumanoidRootPart"); if root then root.CFrame = root.CFrame * CFrame.Angles(0, math.rad(cfg.combat.spinSpeed * 0.5), 0) end end
    if cfg.movement.autoSprint and character then local hum = character:FindFirstChildOfClass("Humanoid"); if hum then hum.WalkSpeed = hum.MoveDirection.Magnitude > 0 and cfg.movement.speedVal or 16 end end
    if cfg.movement.speed and character then local hum = character:FindFirstChildOfClass("Humanoid"); if hum then hum.WalkSpeed = cfg.movement.speedVal end end
    if cfg.movement.jumpPower and character then local hum = character:FindFirstChildOfClass("Humanoid"); if hum then hum.JumpPower = cfg.movement.jumpVal end end
    ...
    if cfg.movement.noClip and character then for _, p in ipairs(character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide = false end end end
    if cfg.movement.thirdPerson and character then local root = character:FindFirstChild("HumanoidRootPart"); if root then Camera.CameraSubject = root; Camera.CameraType = Enum.CameraType.Custom end end
end)

UserInputService.JumpRequest:Connect(function()
    if cfg.movement.infiniteJump and character then local hum = character:FindFirstChildOfClass("Humanoid"); if hum then hum:ChangeState(Enum.HumanoidStateType.Jumping) end end
end)

player.Idled:Connect(function()
    if cfg.movement.antiAfk then local hum = character:FindFirstChildOfClass("Humanoid"); if hum then hum.Jump = true end end
end)
...
local function buildHB(p)
    if p == player then return end; removeHB(p)
    local char, root = p.Character, p.Character and p.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    local hb = Instance.new("Part"); hb.Name = "VortexHitbox"; hb.Size = Vector3.new(cfg.combat.hitboxSize, cfg.combat.hitboxSize, cfg.combat.hitboxSize); hb.Transparency = 1; hb.CanCollide = false; hb.CastShadow = false; hb.Anchored = false; hb.CFrame = root.CFrame; hb.Parent = workspace
    local w = Instance.new("WeldConstraint", hb); w.Part0 = hb; w.Part1 = root
    hbParts[p] = hb
end
RunService.Heartbeat:Connect(function()
    if cfg.combat.hitboxExpander then for _, p in ipairs(Players:GetPlayers()) do if p ~= player and not hbParts[p] then buildHB(p) end end else for _, p in ipairs(Players:GetPlayers()) do removeHB(p) end end
end)
    ...
    if cfg.combat.killAura then
        ...
        if myRoot then
            for _, p in ipairs(Players:GetPlayers()) do
                ...
                if pRoot and pHum and pHum.Health > 0 and (pRoot.Position - myRoot.Position).Magnitude < cfg.combat.killAuraRange then
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if input.KeyCode == Enum.KeyCode.Semicolon then
        if cfg.movement.fly then stopFly() else startFly() end
    end
end)

-- ====== MAIN ======
print("Vortex Hub v" .. VERSION .. " loaded!")
print("Press ; to toggle fly")
